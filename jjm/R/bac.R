...
m1 <- runit("1.0",pdf=FALSE,portrait=F,est=TRUE,exec="src/jjm")
m1 <- readJJM("1.0", path = "config", input = "input")
glimpse(readJJMConfig("1.0", path = "config", input = "input"))

m2.0 <- runit("2.0",pdf=FALSE,portrait=F,est=TRUE,exec="src/jjm")
m2.0 <- readJJM("2.0", path = "config", input = "input")
glimpse(readJJMConfig("2.0", path = "config", input = "input"))

m2.1 <- runit("2.1",pdf=FALSE,portrait=F,est=TRUE,exec="src/jjm")
m2.1 <- readJJM("2.1", path = "config", input = "input")
glimpse(readJJMConfig("2.1", path = "config", input = "input"))

m2.2 <- runit("2.2",pdf=FALSE,portrait=F,est=TRUE,exec="src/jjm")
m2.2 <- readJJM("2.2", path = "config", input = "input")

m3.0 <- runit("3.0",pdf=FALSE,portrait=F,est=TRUE,exec="src/jjm")
m3.0 <- readJJM("3.0", path = "config", input = "input")
summary(readJJMConfig("3.0", path = "config", input = "input"))

m4.0 <- runit("4.0",pdf=FALSE,portrait=F,est=TRUE,exec="src/jjm")
m4.0 <- readJJM("4.0", path = "config", input = "input")
#dev.off()

#summary(m4.0)
plot_selectivities(get_selectivities(m4.0))
age_fits <- get_age_fits(m4.0)
len_fits <- get_len_fits(m4.0)
len_fits
get_len_fits(m4.0) %>% filter(len<150,len>40, fleet_name=="Ind_Chile") |> 
  pivot_longer(predicted:observed) %>% 
  ggplot() +  geom_density(aes(len, value, fill = name),stat = "identity", alpha = 0.3) + 
  facet_grid(year~fleet_name)+ ggthemes::theme_few(base_size = 10)
kobe(c(m4.0,m3.0), engine = "ggplot")

len_fits
age_fits <- get_age_fits(m3.0)
age_fits
summary(m3.0)
?jjmR::summary
diagnostics(m4.0)
args(diagnostics)
report(m3.0, format="html", tangle=TRUE)
plot_selectivities
get_age_fits(m2.0) %>% 
  filter(year > 2010) %>%  pivot_longer(predicted:observed) %>% 
  ggplot() +  geom_density(aes(age, value, fill = name),stat = "identity", alpha = 0.3) + 
  facet_grid(year~fleet_name) + ggthemes::theme_few(base_size = 10)
get_age_fits(m2.2) %>% 
  filter(year > 2010) %>%  pivot_longer(predicted:observed) %>% 
  ggplot() +  geom_density(aes(age, value, fill = name),stat = "identity", alpha = 0.3) + 
...
get_len_fits(m4.0) %>% filter(len<180,len>40) |> 
  pivot_longer(predicted:observed) %>% 
  ggplot() +  geom_density(aes(len, value, fill = name),stat = "identity", alpha = 0.3) + 
  facet_grid(year~fleet_name)+ ggthemes::theme_few(base_size = 10)

recruits <- get_recruits(c(m3.0,m4.0))
recruits <- get_recruits(c(m2,m2.2))
recruits %>% 
  ggplot() + 
  geom_ribbon(aes(year, ymin = lower_recruits, ymax = upper_recruits, fill = model),alpha = 0.5) + 
  geom_line(aes(year, recruits, color = model)) #+ 
  facet_wrap(~model)


tidy_jjm <- tidy_JJM(c(m4.0,m3.0))
index_fits <- tidy_jjm$index_fits
index_fits %>% 
  ggplot() + 
  geom_pointrange(aes(year, observed_ind, ymin = observed_ind - 1.96 * observed_se, ymax =  observed_ind + 1.96 * observed_se, color=model), alpha = 0.5) +
...
index_fits %>% 
  ...
  facet_grid( fleet_name~., scales = "free_x") + 
  scale_x_continuous(name = "Year", guide = guide_axis(n.dodge = 2)) + 
  scale_y_continuous(name = "Standardized Residuals")

fishing_mortality <- get_fishing_mortality(m4.0)
fishing_mortality %>% 
  ggplot(aes(year, mortality, color = age, group = age)) + 
  geom_line() + 
  scale_color_viridis_c() +
  facet_grid(model~stock, scales = "free_y") 
kobe(c(m3.0,m4.0), engine = "ggplot")
qs = get_catchabilities(tidy_jjm)
qs %>% 
  ggplot(aes(year, q, color = model)) + 
  geom_line() + 
  facet_wrap(~ fleet_name, scales = "free_y")

totals <- get_totals(c(m4.0,m2.1,m2.2,m3.0))

totals %>% 
  ggplot(aes(year, value, color = model, linetype = model)) + 
  geom_line(linewidth=1) + 
  facet_grid(metric~., scales = "free_y") + ylim(0,NA)

dev.off()
m2_v_m3 <- combineModels(m3, m2)

  #m0 <- runit("mcomun_1.0",pdf=TRUE,portrait=F,est=TRUE,exec="jjm")

names(m0)
glimpse(get_len_fits(m2.2))
names(get_len_fits(m2.2))

selectivities <- get_selectivities(c(m2.0,m2.2))
glimpse(selectivities)
names(selectivities)
plot_selectivities(selectivities)
?get_selectivities(selectivities)

msy_my_results = get_msy_mt(m2_v_m3)
msy_my_results
kobe(c(m2.2,m3.0), engine = "ggplot")



#-----------------------------------from here down has not been tested for Bacalao--------
# Retro try ----
